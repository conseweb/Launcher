...
@SuppressLint("NewApi") public class QuickService extends Service{
	...
	public void onCreate() {
		...
		initButtonReceiver();showButtonNotify();
	...
	public IBinder onBind(Intent arg0) {
	  ...
	  /**
		...
		//	mRemoteViews.setImageViewResource(R.id.btn_custom_0, R.drawable.ant_settings_ic_launcher);
					 ...
					 if (isWiFi()){
						...
						// mWm = (WifiManager) getSystemService(Context.WIFI_SERVICE);
					       // int wifiState = wifiMgr.getWifiState();
					        ...
					        if (wifiId.equals("unknown")){wifiId=getString(R.string.ant_set_wifi);};
				...
				//	mode=mAudioManager.getRingerMode();
	 		      ...
	 		      //  mBluetoothAdapter = BluetoothAdapter.getDefaultAdapter();
	 		        		  if (mBluetoothAdapter == null) {
	 		        			 ...
	 		        			 mRemoteViews.setImageViewResource(R.id.btn_custom_5, R.drawable.ant_settings_bluetooth_off_nor);
	 		        		  }else{mRemoteViews.setImageViewResource(R.id.btn_custom_5, R.drawable.ant_settings_bluetooth_on_nor);};
			...
			Intent it3 = new Intent(Settings.ACTION_AIRPLANE_MODE_SETTINGS);
			...
			public void onReceive(Context context, Intent intent) {
				...
				if(action.equals(ACTION_BUTTON)){
					...
					case BUTTON_1:
						...
						break;
					case BUTTON_2:
						 if (is3g==false){is3g=true;}else{is3g=false;}
						  mConnectivityManager = (ConnectivityManager)getSystemService(Context.CONNECTIVITY_SERVICE); 
						 if (getMobileDataStatus())  
	                     { 
						...
						break;
					case BUTTON_3:
						 if (iswifi==false){iswifi=true;}else{iswifi=false;}
						 
						 mWm = (WifiManager)getBaseContext().getSystemService(Context.WIFI_SERVICE);  
						 if (isWiFi()){setWifi(false);}else{
						 setWifi(false);
		            	  setWifi(true);}
						// showButton(3);
						break;
					case BUTTON_4:
						switch (isvab){
						case 0:isvab=1;break;
						case 1:isvab=2;break;
						case 2:isvab=0;break;
						default:isvab=0;break;
						}
						mode=mAudioManager.getRingerMode();
		 				 switch (mode){
		 		         case AudioManager.RINGER_MODE_NORMAL:
