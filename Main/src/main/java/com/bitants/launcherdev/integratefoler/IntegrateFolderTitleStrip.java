...
//	private final String TAG = "IntegrateFolderTitleStrip";
	...
	public IntegrateFolderTitleStrip(Context context, AttributeSet attrs, int defStyleAttr) {
...
//		ALog.d("state: " + state);
	...
	private void update(int position, float offset) {
		...
		if ((mRectList.size() > 1) && (position < -1 + mRectList.size())) // 不是第一个并且不是不是最后一个
			mCenterX = mCenterX - offset * (((Rect) mRectList.get(position + 1)).exactCenterX() - ((Rect) mRectList.get(position)).exactCenterX());
...
//			ALog.d("text: " + text);
	...
	private int getClickPosition(float downX, float downY) {
		...
		for (int i = 0; i < mViewPager.getChildCount(); i++) {
			if ((i >= 0) && (i < mRectList.size()) && (((Rect) mRectList.get(i)).contains((int) (downX - mCenterX - halfWidth), 1)))
				return i;
		}
		return -1;
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int width = resolveSize((int) (3.0F * mPaint.measureText(title, 0, title.length())) + getPaddingLeft() + getPaddingRight(), widthMeasureSpec);
		int height = resolveSize((int) (mFontMetrics.bottom - mFontMetrics.top + getPaddingTop() + getPaddingBottom()), heightMeasureSpec);
		setMeasuredDimension(width, height);
		mBaseLine = (getPaddingTop() + (height - getPaddingTop() - getPaddingBottom()) / 2.0F - (mFontMetrics.top + mFontMetrics.bottom) * Math.max(this.mScaleConstant, 1.0F) / 2.0F);
		update(mViewPager.getCurrentItem(), 0.0F);
	}
			...
			if ((pageIndex >= 0) && (pageIndex < pageCount)) {
				...
				if (curRect.intersects(left, 0, right, 1)) {
					...
					if (((CharSequence) pageTitle).length() > MAX_LENGTH)
						pageTitle = ((CharSequence) pageTitle).subSequence(0, MAX_LENGTH - 1) + "...";
...
//                                    " + "+scale);
			...
			canvas.drawText((CharSequence) pageTitle, 0, ((CharSequence) pageTitle).length(), x, y, mPaint);
			if (scale != 1.0F)
				canvas.restore();
		}
