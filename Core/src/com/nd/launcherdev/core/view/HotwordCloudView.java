package com.nd.launcherdev.core.view;
...
public class HotwordCloudView extends FrameLayout implements OnGlobalLayoutListener {
	public final static String TAG = "HotTermCloudView";
	...
	public static final long ANIM_DURATION = 750l;

	/**
	 * 用于随机颜色值 2012.08.07
	 */
	private static final int[] BUILDIN_COLORS = { 0xffff9dd2, 0xff866b6b, 0xff5ec7a8, 0xff33b5e5, 0xffe95a97, 0xff8f4f36, 0xff35A5B3, 0xff475bd4, 0xff542b2b, 0xffd76565, 0xffff7901, 0xffb0a821,
			0xff9bda63, 0xff745328, 0xff79981b, 0xffce2ca2, 0xffE67B75, 0xff9D3438, 0xffF69BAC, 0xff934A43, 0xff5BB3DB };
	...
	public static final int TEXT_SIZE_MIN = 17;
	...
	public HotwordCloudView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public HotwordCloudView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	private void init() {
		...
		random = new Random();
		vecKeywords = new Vector<String>(displayNum);
	...
	public void show(String[] hotwords, int displayNum, int animation, long duration) {
		...
		vecKeywords.clear();
		int addCount = 0, length = hotwords.length;
		if (length < displayNum) {
			addCount = length;
			for (int i = 0; i < addCount; i++) {
				vecKeywords.add(hotwords[i]);
			}
		} else {
			...
			for (int i = 0; i < addCount; i++) {
				...
				vecKeywords.add(list.get(randInt));
				list.remove(randInt);
			}
		}

		this.displayNum = addCount;
	 ...
	 * 设置要展示的热词数
	 */
	public void setHotwordDisplayNum(int num) {
		this.displayNum = num;
	}
	 ...
	 */
	private boolean go2Show(int animType) {
	...
	private boolean show() {
		if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (TextUtils.isEmpty(keyword)) // yuf@2012.11.15 加入非空判断
					continue;

				// 随机颜色
				// int ranColor = 0xff000000 | random.nextInt(0x0077ffff);
				int ranColor = randomColor();
				...
				txt.setOnTouchListener(new TextViewTouchListener(ranColor, 0xff000000));

				txt.setTextSize(TypedValue.COMPLEX_UNIT_SP, txtSize);
				// txt.setShadowLayer(1, 1, 1, 0xdd696969); // 文字阴影
	...
	private int randomColor() {
		...
		int num = BUILDIN_COLORS.length;
		return BUILDIN_COLORS[random.nextInt(num)];
	}
