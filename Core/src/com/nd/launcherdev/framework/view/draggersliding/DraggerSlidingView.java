...
import com.nd.launcherdev.framework.view.commonsliding.CommonSlidingView;
...
import com.nd.launcherdev.framework.view.draggersliding.datamodel.IDraggerData;
...
public abstract class DraggerSlidingView extends CommonSlidingView implements
	...
	public void startDrag(View view, int positionInData, int positionInScreen,
		...
		mDragController.startDragFromSlidingView(view, this, dragInfo, list);
	}
	...
	protected void handleMoveAnimation() {
		...
		removeViewSafely(mOriginalView);
		if (mOriginalScreen != getCurrentScreen()) {
	...
	protected void handleFallbackAnimation(boolean animate) {
		...
		if (mOriginalFixedScreen != mOriginalScreen) {
				...
				for (int screen = mOriginalFixedScreen; screen > mOriginalScreen; screen--) {
					...
					if (screen == mOriginalScreen + 1) {
						if (screen == getCurrentScreen() + 1) {
							/**
							 * 延时布局，延迟时间与DraggerLayout中位移动画时间相同，
							 * 避免图标正在做移动动画(单页布局)的同时进行回退动画，从而产生图标重叠的异常显示情况
							 */
							final DraggerLayout finalPreviousLayout = previousLayout;
							handler.postDelayed(new Runnable() {

								@Override
								public void run() {
									finalPreviousLayout.startViewAnimation(finalPreviousLayout.getChildAt(finalPreviousLayout.getChildCount() - 1), null, true, pageWidth);
								}
				...
				for (int screen = mOriginalFixedScreen; screen < mOriginalScreen; screen++) {
					...
					if (screen == mOriginalScreen - 1) {
						if (screen == getCurrentScreen() - 1) {
							 ...
							 * 避免图标正在做移动动画(单页布局)的同时进行回退动画，从而产生图标重叠的异常显示情况
							 */
							final DraggerLayout finalNextLayout = nextLayout;
							handler.postDelayed(new Runnable() {

								@Override
								public void run() {
									finalNextLayout.startViewAnimation(finalNextLayout.getChildAt(0), null, true, -pageWidth);
								}
							}, MOVE_ANIMATION_DURATION);
						} else {
							nextLayout.reLayout();
			...
			if (animate && mOriginalScreen == getCurrentScreen()) {				
				 ...
				 * 避免图标正在做移动动画(单页布局)的同时进行回退动画，从而产生图标重叠的异常显示情况
				 */
				final DraggerLayout finalCurrentLayout = currentLayout;
				handler.postDelayed(new Runnable() {

					@Override
					public void run() {
						finalCurrentLayout.startViewAnimation(mOriginalView);
					}
				}, MOVE_ANIMATION_DURATION);
			} else {
				currentLayout.reLayout();
