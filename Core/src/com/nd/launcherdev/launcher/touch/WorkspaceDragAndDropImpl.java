...
import com.nd.launcherdev.framework.AnyCallbacks.OnDragEventCallback;
import com.nd.launcherdev.kitset.util.MessageUtils;
...
import com.nd.launcherdev.launcher.screens.dockbar.BaseMagicDockbar;
import com.nd.launcherdev.launcher.screens.preview.PreviewCellInfo;
import com.nd.launcherdev.launcher.support.BaseCellLayoutHelper;
import com.nd.launcherdev.launcher.touch.Alarm;
import com.nd.launcherdev.launcher.touch.BaseDragController;
import com.nd.launcherdev.launcher.touch.DragSource;
import com.nd.launcherdev.launcher.touch.WorkspaceDragAndDrop;
import com.nd.launcherdev.launcher.touch.outline.DragHelper;
...
public class WorkspaceDragAndDropImpl implements WorkspaceDragAndDrop{
	...
	protected OnDragEventCallback dragOverViewFolder = null; //拖动时覆盖的view，用于合成文件夹
	...
	public boolean acceptDrop(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView, Object dragInfo){
		...
		if(!mWorkspace.isAllAppsIndependence((ItemInfo) dragInfo)){//防止拖动应用列表图标到满屏时丢失
			if(source instanceof BaseMagicDockbar || mDragController.isDragFromFolder(source)){
				...
				if(dragOverView != null && dragOverView instanceof OnDragEventCallback && !mWorkspace.isRealFolder(dragOverView.getTag())
	...
	public void onDragExit(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView, Object dragInfo){
		...
		if ((dragInfo instanceof ApplicationInfo  || dragView.getDragingView() instanceof OnDragEventCallback)
			 && mLastDragOverView != null) {
			if (mLastDragOverView instanceof OnDragEventCallback) {
				...
				if(!lp.isOnReorderAnimation && !lp.isOnPending){					
					((OnDragEventCallback) mLastDragOverView).onExitAni(dragView);
				}
			}
		}
		//离开文件夹时清除轮廓
	...
	public boolean handleMergeFolderAndWidget(DragSource source, int x, int y, int xOffset, int yOffset, 
		...
		boolean allowHandleMerg = (dragInfo instanceof ApplicationInfo || dragView.getDragingView() instanceof OnDragEventCallback) 
				&& (dragOverView instanceof OnDragEventCallback);
		if(dragOverView != null && dragOverView.getVisibility() == View.VISIBLE){
			...
			if(allowHandleMerg){//dragOverView文件夹处理
				dragOverViewFolder = (OnDragEventCallback)dragOverView;
				...
				if (dragOverView != mDragController.getOriginator() //防止被拖动view也产生文件夹动画
					...
					&& allowAnimateMergeFolder) {
						...
						((OnDragEventCallback) dragOverView).onEnterAni(dragView);
						//进入文件夹时，清除轮廓
						DragHelper.getInstance().dragOutlineAnimateOut();
				...
				if(targetCellDistance >= mMaxDistanceForFolderCreation && mDragMode == DRAG_MODE_FOLDER ){
					if (mLastDragOverView != null && dragOverView instanceof OnDragEventCallback) {
						mWorkspace.destoryCurrentChildHardwareLayer();
						((OnDragEventCallback) mLastDragOverView).onExitAni(dragView);
					} 
					mDragMode = DRAG_MODE_NORMAL;
	...
	public void restoreFolderAnimation(){
		try{
			if(mDragMode == DRAG_MODE_FOLDER && mLastDragOverView != null && mLastDragOverView instanceof OnDragEventCallback){
				mWorkspace.destoryCurrentChildHardwareLayer();
				((OnDragEventCallback) mLastDragOverView).onExitAni(null);
				mDragMode = DRAG_MODE_NORMAL;
			}
		}catch(Exception e){
			e.printStackTrace();
