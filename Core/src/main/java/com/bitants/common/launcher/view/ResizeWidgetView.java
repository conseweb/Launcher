...
package com.bitants.common.launcher.view;
...
public class ResizeWidgetView extends View {
	...
	public void draw(Canvas canvas) {
		...
		try {
			...
			//e.printStackTrace();
			int screenWidth = ScreenUtil.getCurrentScreenWidth(getContext());
			int screenHeight = ScreenUtil.getCurrentScreenHeight(getContext());
			Rect topRect = new Rect(0,0,screenWidth,mDrawRect.top);
			Rect leftRect = new Rect(0,mDrawRect.top,mDrawRect.left,mDrawRect.bottom);
			Rect rightRect = new Rect(mDrawRect.right,mDrawRect.top,screenWidth,mDrawRect.bottom);
			Rect bottom = new Rect(0,mDrawRect.bottom,screenWidth,screenHeight);
			canvas.drawRect(topRect, mNoFocusPaint);
			canvas.drawRect(leftRect, mNoFocusPaint);
			canvas.drawRect(rightRect, mNoFocusPaint);
			canvas.drawRect(bottom, mNoFocusPaint);
		}
        if(mCollision){
            try {
				...
				canvas.drawRect(rightRect, mNoFocusPaint);
				canvas.drawRect(bottom, mNoFocusPaint);
				canvas.drawRect(mDrawRect, mFocusPaint);
			}
        }
	...
	public int getHit(float x, float y) {
		...
		final float hysteresis = 35F;//修改触发缩放的灵敏度  by guojy
	...
	public void setup(Matrix m, Rect imageRect, RectF cropRect, boolean circle, boolean maintainAspectRatio, float minWidth, float minHeight) {
		if (circle) {
	 ...
	 * 是否需要触发点击非小插件区域事件
	 */
	private boolean mShouldTriggerClickOutside = false;
	private OnClickOutsideListener mOnClickOutsideListener = null;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			float x = event.getX();
			float y = event.getY();
			int widthExtend = mResizeDrawableWidth.getIntrinsicWidth() / 2;
			int heightExtend = mResizeDrawableHeight.getIntrinsicHeight() / 2;
			Rect rect = new Rect(mDrawRect.left - widthExtend, mDrawRect.top - heightExtend, mDrawRect.right + widthExtend, mDrawRect.bottom + heightExtend);
			if(!rect.contains((int)x, (int)y)) {
				mShouldTriggerClickOutside = true;
			}
			int edge = getHit(x, y);
			mMotionEdge = edge;
			if (edge != GROW_NONE) {
		...
		case MotionEvent.ACTION_UP:
			if(mShouldTriggerClickOutside && null != mOnClickOutsideListener) {
				mOnClickOutsideListener.onTrigger();
				mShouldTriggerClickOutside = false;
				break;
			}
